...
$sql = "SELECT barcode , sum(qty) as qty FROM barcodes where user_id = '". $_SESSION['userId'] ."' AND qty != 0 GROUP BY barcode";
$result = mysqli_query($conn, $sql);

// Set headers to force download
header('Content-Type: text/csv');
header('Content-Disposition: attachment;filename="'. $_SESSION['firstName'] .' '. $_SESSION['lastName']  .'" ST.csv');
