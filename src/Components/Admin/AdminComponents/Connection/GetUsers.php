...
if (isset($_SESSION['isLoggedIn']) && $_SESSION['isLoggedIn'] === true && $_SESSION['roll'] === 'Admin') {
    ...
    $sql = "SELECT * FROM users where user_id >= '". $_SESSION['userId'] ."' AND account_id='". $_SESSION['accountId'] ."' ";
        ...
        if ($stmt->error) {
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                            ...
                            $barcodessql = "SELECT COUNT(DISTINCT barcode) as scanned FROM barcodes WHERE user_id = '". $row['user_id'] ."'";
                            $barcoderesult = $conn->query($barcodessql);
                            // Check if a matching user was found
                            if ($barcoderesult->num_rows > 0) {
                                // Fetch user details
                                $barcode = $barcoderesult->fetch_assoc();
                                $userqty = $barcoderesult->num_rows;
